...
proc calc_PISEMA {frame} {
  ...
  foreach res $resids_CS {
    set atomC [atomselect top "segname PROB and resid [expr {$res-1}] and name C"]
    set atomN [atomselect top "segname PROB and resid $res and name N"]
    set atomH [atomselect top "segname PROB and resid $res and name HN"]
...
set outStream [open dc_sim.chainB.xvg w]
foreach res $resids_CS {
...
set outStream [open cs_sim.chainB.xvg w]
foreach res $resids_CS {
